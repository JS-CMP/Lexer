...
TEST(AutoGeneratedTests /*unused*/, AllCases /*unused*/) {
    std::string baseDir = "../tests/cases/basic/harness/propertyHelper.js";

    std::vector<fs::path> js_files;
    if (fs::is_directory(baseDir)) {
        fs::recursive_directory_iterator iter(baseDir);
        for (const auto& entry : iter) {
            js_files.push_back(entry.path());
        }
    } else {
        js_files.push_back(fs::path(baseDir));
    }

    for (const auto& entry : js_files) {
        if (entry.extension() == ".js") {
            std::string js_path = entry.string();
            std::filesystem::path cpp_path = entry;
            cpp_path.replace_extension(".out");
            std::filesystem::path out_path = entry;
